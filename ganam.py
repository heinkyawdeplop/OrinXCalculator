...
class JSONBinDB:
    def __init__(self):
        # Railway Environment Variables (စာလုံးအတိအကျ)
        self.api_key = os.environ.get("JSONBIN_API_KEY")
        self.bin_id = os.environ.get("JSONBIN_BIN_ID")
        self.users = {}
        self.groups = {}
        self.start_time = time.time()
        
        # Debug: Railway က variable ရောက်လားစစ်မယ်
        print(f"🔍 API Key from env: {'✅ Found' if self.api_key else '❌ Not found'}")
        print(f"🔍 BIN ID from env: {'✅ Found' if self.bin_id else '❌ Not found'}")
        
        if self.api_key and self.bin_id:
            print("✅ JSONBin.io connected (from Railway)")
            self.load_data()
        else:
            print("⚠️ No JSONBin config, using local file")
            self.load_local()
